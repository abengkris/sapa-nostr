...
import { Search, Loader2, User as UserIcon, TrendingUp, X, CheckCircle2 } from "lucide-react";
      ...
      <div className="p-0">
        ...
        {!debouncedQuery && !loading && (
          <div className="animate-in fade-in duration-500">
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-sm max-w-xs mx-auto">Explore the Nostr network. Search for names, #hashtags, or keywords.</p>
            </div>
            ...
            <div className="flex overflow-x-auto pb-4 gap-4 scrollbar-hide -mx-2 px-2">
              {profiles.map((user) => (
                <Link
                  key={user.pubkey}
                  href={`/${user.npub}`}
                  className="flex flex-col items-center min-w-[130px] p-4 rounded-3xl bg-gray-50 dark:bg-gray-900/50 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-transparent hover:border-blue-100 dark:hover:border-blue-900/30 transition-all text-center group"
                >
        ...
        {posts.length > 0 && (
          <section className="pb-20 animate-in fade-in duration-500">
            <h2 className="text-sm font-black uppercase tracking-widest text-gray-400 p-6">Posts & Articles</h2>
            ...
            {hasMore && (
              <div className="p-10 text-center border-t border-gray-100 dark:border-gray-900">
                <button 
                  onClick={() => loadMore()}
                  disabled={loading}
